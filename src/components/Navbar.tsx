...
export default function Navbar() {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center z-50">
        <Link href="/">
          <Image
            ...
            width={160}
            height={64}
            className="w-auto h-20 md:h-20 object-contain transition-all duration-300 brightness-0"
            priority
          />
        </Link>
      </div>
        ...
        {navLinks.map((link) => {
          ...
          return (
            <Link
              ...
              className="group text-xl transition-all duration-200 flex flex-col items-center text-black hover:text-brand-blue"
            >
              <span className="flex flex-col items-center">
                <span className={`transition-all duration-200 ${
                  isActive ? "font-bold opacity-100" : "font-light opacity-80 group-hover:font-bold group-hover:opacity-100"
                }`}>
                  {link.name}
                </span>
      ...
      {/* Mobile Menu Overlay */}
      <div className={`fixed inset-0 bg-white z-40 flex flex-col items-center justify-center space-y-10 transition-transform duration-500 md:hidden ${
          ...
          return (
            <Link
              ...
              className={`text-3xl transition-all duration-200 ${
                isActive ? "font-bold text-brand-blue" : "font-light text-black"
