...
function LoadingSkeleton() {
  return (
    <div className="flex gap-4 sm:gap-4 lg:gap-4">
      {[...Array(4)].map((_, col) => (
        <div key={col} className={`flex flex-col gap-4 flex-1 ${col >= 2 ? "hidden md:flex" : ""} ${col >= 3 ? "hidden lg:flex" : ""}`}>
          {[...Array(3)].map((_, i) => (
            <Skeleton
              key={i}
              className={`w-full rounded-[20px] ${(col + i) % 3 === 0 ? "aspect-[3/4]" : (col + i) % 3 === 1 ? "aspect-[4/3]" : "aspect-square"}`}
            />
          ))}
        </div>
      ))}
...
/** 
 * Pinterest-style Individual image card:
...
function GalleryCard({
  ...
  return (
    <div
      ...
      data-aos-once="true"
      className="relative group cursor-pointer overflow-hidden rounded-[20px] bg-gray-50 transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] hover:shadow-xl"
      onClick={onClick}
    >
      ...
      <Image
        ...
        className={`w-full h-auto block transition-all duration-1000 group-hover:scale-105 group-hover:brightness-90 ${
      ...
      <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex items-center justify-center z-20">
        <span className="text-white text-xs font-semibold px-4 py-2 bg-white/10 backdrop-blur-sm rounded-full transform translate-y-2 group-hover:translate-y-0 transition-all duration-500">
          View Detail
        </span>
      </div>
    </div>
...
export default function GalleryPage() {
  ...
  return (
    <main className="pt-40 sm:pt-32 md:pt-40 lg:pt-40 xl:pt-52 pb-16 min-h-screen">
        ...
        <div className="max-w-[1600px] mx-auto">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-8 md:gap-12 mb-12">
            <div className="w-full md:w-1/2" data-aos="fade-up">
              <h1 className="font-montserrat text-[#0066B2] text-[clamp(28px,4vw,44px)] font-bold tracking-tight">
                Gallery
              </h1>
            </div>
            <div className="w-full md:w-1/2 lg:max-w-[500px]" data-aos="fade-up" data-aos-delay="100">
              <p className="font-poppins font-[400] text-gray-500 text-base sm:text-lg leading-relaxed">
                Explore our completed projects showcasing quality and precision.
              </p>
            </div>
          </div>
                  ...
                  <div key={ci} className="flex flex-col gap-4 flex-1">
                    {col.map(({ img, globalIndex }) => (
                      <GalleryCard
                        ...
                        aosDelay={(globalIndex % colCount) * 100}
              ...
              {(totalKnownPages > 1 || hasMore) && (
                <div className="mt-16 flex items-center justify-center gap-2 flex-wrap">
                  ...
                  <button
                    ...
                    className="w-10 h-10 flex items-center justify-center rounded-full border border-gray-100 text-gray-400 hover:border-[#0066B2] hover:text-[#0066B2] transition-all disabled:opacity-20"
                  >
                    ‹
                  </button>
